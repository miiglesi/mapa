...
if (typeof ty === "undefined") {
	var ty = {
		chainOnLoad: function(onLoadHandler) {
			...
			{ oldOnLoad && oldOnLoad(); onLoadHandler(); }
			)(window.onload);
		},
		loadStyleSheet: function(url) {
			if (document.createStyleSheet)
				...
				{ var linkEl = document.createElement("link"); linkEl.rel = "stylesheet"; linkEl.href = encodeURI(url); 	document.getElementsByTagName("head")[0].appendChild(linkEl); }
				else
...
if (ty.trustScoreLoaded == undefined) {
	...
	{ ty.loadStyleSheet("//api.trustyou.com/static/3rdparty/cleanslate/cleanslate.css?v=v5.35.3-12-g3164ba4"); ty.loadStyleSheet("//api.trustyou.com/static/css/embedded/seal.css?v=v5.35.3-12-g3164ba4"); ty.loadStyleSheet("//fonts.googleapis.com/css?family=Roboto:400,300,500,700"); ty.loadStyleSheet("//cdn.trustyou.com/apps/commons/public/css/trustyou-iconic.latest.css"); }
	);
...
/**
 * @description funcion para generar el link de la red social donde compartir
 * @param {object} data - datos necesarios para generar la url del link
 */
...
/**
 * @description function que construye el link para compartir en facebook
 * @param {object} - objeto con los datos necesarios para constuir el link
 */
...
/**
 * @description function que construye el link para compartir en twitter
 * @param {object} - objeto con los datos necesarios para constuir el link
 */
...
var utils = function () {
  ...
  function callTrackForms(t, e, i) {
    /*try {
        //seguimientoForms esta funcion esta dentro de uatag.js que se carga asincronamente
        //formsAnalysys esta en m-track-data.js
        //formSuccess esta en m-track-data.js
        //formSuccess esta en m-track-data.js
    ...
    if (typeof geoIpData !== "undefined") {
      if (!geoIpData.country) {
        return $.ajax({
          ...
          console.log("[getGeoIpData] : error petición dataGEOIP");
          return geoIpData;
        });
      } else {
  ...
  function initializeClock($clock) {
    ...
    var time = {
      ...
      get endTime() {
        if (this.hasEndTime) {
          ...
          return new Date(this.now + (this.mins * 60 * 1000) + (this.secs * 1000));
        }
      },
      get hasEndTime() {
    ...
    if (typeof pageData !== 'undefined') {
      //EY arobles: Cambio condición para calcular el length porque pageData.clockCountdown.length es undefined
  ...
  //calcula la distancia entre dos coordenadas y retorna la distancia en km
  function getDistanceBetweenCoords(origin, end) {
    ...
    var c;
    try {
      a = Math.pow(Math.sin(dlat / 2.0), 2) + Math.cos(origin.lat * d2r) * Math.cos(end.lat * d2r) * Math.pow(Math.sin(dlong / 2.0), 2);
    ...
    try {
        ...
        var a = Math.pow(Math.sin(dlat / 2.0), 2) + Math.cos(h1Lat * d2r) * Math.cos(h2Lat * d2r) * Math.pow(Math.sin(dlong / 2.0), 2);
  ...
  function removeDiacritics(str, defaultDiacritics) {
    let defaultDiacriticsMap = (defaultDiacritics) ? defaultDiacritics : defaultDiacriticsRemovalMap;
  ...
  function addThousandsSeparator(value) {
    var retVal = value ? parseFloat(value.toString().replace(/,/g, "")) : 0;
    return retVal.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".");
  }

  /*
  su uso en en checkout.js pero esta comentado porque no se sabe si realmente es necesario. Cuando se implemente checkout habrá que revisar y si no borrar.
  function changeInputValidation = function(inputs, isRequired, options) {
    if (inputs) {
        var $inputs = $(inputs);
        var executeSubmit = options || {
            submitting: true
        };
        if (isRequired) {
            $inputs.attr("required", "required");
        } else {
            $inputs.removeAttr("required", "required").removeData("validationRequiredMessage");
        }
        $inputs.jqBootstrapValidation();
        $inputs.trigger("change.validation", options);
    }
};
  */
  ...
  function loadGoogleMapScript() {
    var deferred = $.Deferred();
    // EY, incluimos api ruta maps para que no falle mapas
    ...
    if (typeof Globals != "undefined") {
      ...
      // añado places si los necesito, en hotel page -> location -> how to arrived
      if ($(".how-to-arrive").length) srcGooglemaps = srcGooglemaps + "&libraries=places"
    }
    $.getScript(srcGooglemaps).done(() => {
      deferred.resolve();
    }).fail(() => {
      deferred.reject();
    });
  ...
  function isValidDates() {
    ...
    var dates = {
      ...
      get currentFormated2() {
        //return this.current.format("DD/MM/YY");
    ...
    //las fechas son validas si enDate es mayor que initDate, si no es mayor de 15 días y si la fecha actual es mayor que la fecha init
    if (globalConfig.urlParameters.fini && globalConfig.urlParameters.fout) {

      //isValid = (0 < dates.diff < 15) && dates.initFormated.isBefore(dates.currentFormated) && (dates.initFormated.isBefore(dates.endFormated));
      ...
      //isValid = (0 < dates.diff < 15) && dates.initFormated.isSameOrBefore(dates.currentFormated) && (dates.initFormated.isBefore(dates.endFormated));
    }
...
// Inits Bootstrap Affix for the given selectors
$(function () {
  if (isIEorEdge().isIE) {
    utils.getAffixTopOffset($('section.m-book-now'));
    utils.getAffixTopOffset($('section.m-search-box'));
  }
  // Init affix for all browsers. Useful when module needs to change style when sticked (thx to the changing classes of bootstrap)
  if ($('.p-hotel .m-book-now').length) {
    var offsetTopPhotel = $('.p-hotel .m-book-now').offset().top;
    $('.p-hotel .m-book-now').affix({ offset: { top: offsetTopPhotel } });
  }
});


// Soporte de método forEach para IE11 (NodeList)
...
// ------------------------------------------------------------------------------------
// Ey, reemplazar funcion tConvert
// ------------------------------------------------------------------------------------
...
// ------------------------------------------------------------------------------------
// Recolocar desplegable según espacio vertical disponible
// ------------------------------------------------------------------------------------
function dropdownPosition(input, dropdown, margin) {

  // Posición del input
  // EY AÑDIR el alto de la caja
  let positionInput = ($(input).offset().top + $(input).outerHeight()) - $(document).scrollTop();

  // Espacios disponbles arriba y abajo del input
  let topSpace = positionInput > $(dropdown).outerHeight();
  let bottomSpace = $(window).height() - positionInput < $(dropdown).outerHeight();

  // Controlamos la posición (si no hay espacio en ninguno va hacia abajo)
  (bottomSpace && topSpace)
    ? $(dropdown).offset({ top: $(input).offset().top - $(dropdown).outerHeight() - margin })
    : $(dropdown).offset({ top: $(input).offset().top + $(input).outerHeight() + margin });

  // ( bottomSpace && topSpace )
  //   ? $(dropdown).css('top', -( $(dropdown).outerHeight() ))
  //   : $(dropdown).css('top', $(input).outerHeight() + margin );
}



// ------------------------------------------------------------------------------------
// Polyfill includes para ie11
// ------------------------------------------------------------------------------------
...
/**
  * @description Recoge info de navegador y sistema operativo en un objeto
  * @return - Objecto con info de navegador y sistema operativo
  */
...
/**
  * @description Navegar a página anterior en flechas atrás de móvil
  */
 $(document).ready(function () {
  $('.step-back').on('click touchstart', function() {
    history.go(-1);
...
/*  ----------------------------------------------------------------------------------------------------
    JS ADN global para todo el proyecto
------------------------------------------------------------------------------------------------------ */
...
-------------------------------------------------- */
var globalConfig = {
    "elements": { //aquí vamos a recoger todos los elementos que aparecent en muchas páginas para tenerlos cacheados y darles funcionalidad
        "$rewardsSubmenu": $('.js-rewards-submenu'),//menu rewards de la cabecera
        "$openModal": $("[data-toggle='modal'], [data-toggle='external-modal'], .btn-modal-iframe, .js-recorverpass-modal-trigger"), //abría que añadir una clase para recoger los elementos que abren modal mediante un click sin el data-toggle
        "$selectsElements": $(".selectpicker"), //selectores select para aplicarles el plugin boostrap-select (sería mejor hacerlo por página y no global?)
        "$shareButtons": $('.js-share, .js-share-img'), //botones / links para compartir en redes sociales (si aparecen en el footer siempre sería mejor pasarlos al async)
        "$cookiesMessage": $('.cookiesLanding'), //mesaje de cookies
        "$backgroundElements": $('[data-background]'), //elementos a los que se va ha insertar un background image (quizas haya que buscarlos por página)
        "$squareElements": $('.js-square'), // elementos a los que se le va a dar el tamaño de un cuadrado (quiza haya que buscarlos por página)
        "$affixedElements": $('[data-spy="affix"]'),
        "$paginateElements": $('.paginate-me')
    },
    windowWidth: $(window).width(), //guardamos el tamaño de la pantalla al inicializar
    windowHeight: $(window).outerHeight(), //guardamos el tamaño de la pantalla al inicializar
    xxsBreak: 479,
    xsBreak: 767,
    smBreak: 991,
    mdBreak: 1199,   
...
-------------------------------------------------- */
$(function() {
    //inicializamos los select con el plugin bootstrap-select
    ...
    //inicializamos el submenu de rewards con el plugin affix para que se quede fijo cuando hagamos scroll
    //TODO: como se dijo que no habia elementos de cabecera fijos, se comenta y se debe eliminar el affix.js
    /*if(globalConfig.elements.$rewardsSubmenu.length) {
        globalConfig.elements.$rewardsSubmenu.affix({
            offset: {
                // Cálculo del offset top
                top: globalConfig.elements.$rewardsSubmenu.offset().top
            }
        });

        //Affix header rewards: calcula el offset-top para el sticky menu
        $(window).on('resize', function () {
            var newOffset;
            if ($('.top-nav-01').is(':visible')) {
                newOffset = $('.top-nav-01').outerHeight() + $('.menuNav-one').outerHeight();
            } else {
                newOffset = $('.menuNav-one').outerHeight();
            }
            globalConfig.elements.$rewardsSubmenu.data('bs.affix').options.offset = newOffset;
        });
    }*/
...
//TEMP: Affix header rewards
/*if ($('.js-rewards-submenu').length){
  $('.js-rewards-submenu').affix({
    offset: {
      // Cálculo del offset top
      top: $('.js-rewards-submenu').offset().top
    }
  });
}*/

/**
 * @description aplica ciertos estilos a los elementos si la pantalla es menor de 1000px
   TO DO: Revisar si es necesario en V3
 */
function fixedModalsInIpad() {
    if(globalConfig.windowWidth <= 1000) {
        $("html").css({
            "overflow": "scroll",
            "overflow-y": "auto",
...
/**
 * @description modulo para aplicar la misma altura a un set de elmenentos
 */
...
/**
 * @description Inserta imágenes como background
 */
function setElementBackground(){
    ...
    globalConfig.elements.$backgroundElements.each(function (){
      ...
      /*
      if (is_retina_device() === true && $this.attr('data-background-2x')){
...
var isIEorEdge = function(uaString) {
  var browsers = {isIE: false, isEdge: false};

  uaString = uaString || navigator.userAgent;
  var match = /\b(MSIE |Trident.*?rv:|Edge\/)(\d+)/.exec(uaString);

  if (match){
    // Everything above 11 is Edge
    if(parseInt(match[2]) <=11) browsers.isIE = true;
    else browsers.isEdge = true;
  }
  return browsers;
}

/**
 * @description resuelve el comportamiento de los elementos con position fixed
 */
    ...
    if (globalConfig.elements.$affixedElements.length > 0 ){
        ...
        globalConfig.elements.$affixedElements.each(function() {
            ...
            if($parent.attr("class") && $parent.attr("class").indexOf("affix-holder") >= 0) {
                ...
                //version tablet: cuando se recarga la página con scrool distinto de 0 quitar la altura al affix
 ...
 * @return {[int]} [Width of the scroll]
 */
...
$(function() { // DOM Ready
    //que hacemos cuando carga el window
    $(window).on('load', function() {
        initHelpers(); //inicializamos los helpers
        //fixAffixedElements();
    });

    //que hacemos cuando se redimensiona el window
    $(window).on('resize', function() {
